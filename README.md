# Wakarana
WakaranaはPHPのwebアプリケーションへの組み込みを想定した著作権フリー(パブリックドメイン)のユーザー管理・ログイン認証ライブラリです。  
基本的なログイン認証機能とツリー構造で割り当て可能な権限を使用したロールベースアクセス制御に加え、TOTPによる2要素認証や、ワンタイムトークンによるCSRF(クロスサイトリクエストフォージェリ)対策をサポートしており、少人数開発でもこれらの機能を手軽に利用できるよう設計されています。  
